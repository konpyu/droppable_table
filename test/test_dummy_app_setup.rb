...
class TestDummyAppSetup < Minitest::Test
  ...
  def test_activerecord_descendants_accessible
    ...
    assert descendants.any? { |model| model.name == "Vehicle" }, "Vehicle should be in descendants. Found: #{model_names}"
    assert descendants.any? { |model| model.name == "Car" }, "Car should be in descendants. Found: #{model_names}"
    assert descendants.any? { |model| model.name == "CustomTableModel" }, "CustomTableModel should be in descendants. Found: #{model_names}"
    assert descendants.any? { |model| model.name == "SecondaryUser" }, "SecondaryUser should be in descendants. Found: #{model_names}"
  end
end